...
class LogController extends GetxController {
  ...
  Future<void> fetchAppenders() async {
    var payload = await RestClient.rpc(RPC.rpcLogAppenderList);
  ...
  Future<void> addFileAppender() async {
    ...
    var payload = await RestClient.rpc(RPC.rpcLogAppenderAdd, parameters: reqParam);
    if (payload.isSuccess()) {
  ...
  Future<void> removeAppender(int id) async {
    var reqParam = {'id': id};
    var payload = await RestClient.rpc(RPC.rpcLogAppenderRemove, parameters: reqParam);
    if (payload.isSuccess()) {
      await fetchAppenders();
      clear();
    } else {
  ...
  Future<void> fetchSystemLog() async {
    var payload = await RestClient.rpc(RPC.rpcLogSystem);
    if (payload.isSuccess()) {
