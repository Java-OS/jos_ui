...
class KernelController extends GetxController {
  ...
  Future<void> fetchKernelParameters() async {
    ...
    var payload = await RestClient.rpc(RPC.rpcSystemKernelParameterList);
  ...
  Future<void> fetchConfiguredKernelParameters() async {
    ...
    var payload = await RestClient.rpc(RPC.rpcSystemKernelParameterGet);
    if (payload.isSuccess()) {
      var json = jsonDecode(payload.content!) as Map;
  ...
  Future<void> setKernelParameter() async {
    ...
    var payload = await RestClient.rpc(RPC.rpcSystemKernelParameterSet, parameters: param);
    if (payload.isSuccess()) {
  ...
  Future<void> unsetKernelParameter(String key) async {
    ...
    var payload = await RestClient.rpc(RPC.rpcSystemKernelParameterUnset, parameters: {'key': key});
    if (payload.isSuccess()) {
      await fetchConfiguredKernelParameters();
  ...
  Future<void> fetchKernelModules() async {
    ...
    var payload = await RestClient.rpc(RPC.rpcSystemKernelModuleList);
    if (payload.isSuccess()) {
  ...
  Future<void> loadKernelModule() async {
    ...
    var payload = await RestClient.rpc(RPC.rpcSystemKernelModuleLoad, parameters: param);
    if (payload.isSuccess()) {
  ...
  Future<void> unloadKernelModule(String name) async {
    ...
    var payload = await RestClient.rpc(RPC.rpcSystemKernelModuleUnload, parameters: {'name': name});
    if (payload.isSuccess()) {
